          ...
          <Card className="mb-8 p-8 rounded-3xl border-[#2E2E2F]/10">
            ...
            <div className="mt-8 pt-8 border-t border-[#2E2E2F]/10">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { label: 'Total Events', val: currentSubscription.plan.limits?.max_events },
                  { label: 'Active Events', val: currentSubscription.plan.limits?.max_active_events },
                  { label: 'Staff Accounts', val: currentSubscription.plan.limits?.max_staff_accounts },
                  { label: 'Monthly Attendees', val: currentSubscription.plan.limits?.max_attendees_per_month },
                ].map((item, idx) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {availablePlans.map((plan) => {
            ...
            return (
              <Card
                key={plan.planId}
                className={`p-6 rounded-3xl border-[#2E2E2F]/10 ${plan.isRecommended ? 'ring-2 ring-[#38BDF8] ring-offset-4' : ''}`}
              >
                {plan.isRecommended && (
                  <div className="bg-[#38BDF8] text-white text-xs font-black px-3 py-1 rounded-full inline-block mb-4">
                    RECOMMENDED
                  </div>
                )}

                <h3 className="text-xl font-black text-[#2E2E2F] mb-1">{plan.name}</h3>
                <p className="text-sm text-[#2E2E2F]/60 mb-4">{plan.description}</p>

                <div className="mb-6">
                  <span className="text-3xl font-black text-[#2E2E2F]">₱{Number(price || 0).toLocaleString()}</span>
                  <span className="text-sm text-[#2E2E2F]/50">/{billingCycle === 'monthly' ? 'mo' : 'yr'}</span>
                </div>
                ...
                <div className="space-y-3 mb-6">
                  {[
                    ...
                    <div key={idx} className="flex items-center gap-2">
                      {feature.enabled ? (
                        <ICONS.CheckCircle className="w-4 h-4 text-green-500" strokeWidth={3} />
                      ) : (
                        <ICONS.XCircle className="w-4 h-4 text-[#2E2E2F]/30" strokeWidth={3} />
                      )}
                      <span className={`text-sm ${feature.enabled ? 'text-[#2E2E2F]' : 'text-[#2E2E2F]/40'}`}>
                        {feature.label}
                      </span>
                    </div>
                  ))}
                ...
                <div className="space-y-2 mb-6">
                  <div className="flex justify-between text-sm">
                    <span className="text-[#2E2E2F]/50">Events</span>
                    <span className="font-bold">{plan.limits?.max_events}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-[#2E2E2F]/50">Monthly Attendees</span>
                    <span className="font-bold">{plan.limits?.max_attendees_per_month}</span>
                  </div>
                </div>
                ...
                <Button
                  ...
                  className={`w-full ${isCurrentPlan ? '!bg-green-500 !text-white' : ''}`}
                >
