...
CREATE INDEX IF NOT EXISTS idx_settings_lookup_composite ON public.settings(user_id, key);
...
CREATE POLICY "Admins can view global settings" ON public.settings
    FOR SELECT
    USING (
        EXISTS (
